...
void check()
{
	...
	if(x[0]<100)
		...
		if(punkt_x/5>0&&punkt_x/5<(8)*350)
		{
			if(time==0)
			{
				LPC_TIM0->TCR = 1;
				//trzeba zrobic start timera
				for(i=0;i<8;i++)
				{
					if(punkt_x/5>k*350 && punkt_x/5<(k+1)*350)
					{
						dzwiek=f[k];
					}
				}
			...
			if(msTicks3>0)
			{
				...
				if(zapis)
				{
					...
					for(int i =0;i<zapis;i++){
						zagraj2(melody[i].dzwiek,msTicks3);
						melody[i].dzwiek =0;
					}
					zapis=0;
				}
